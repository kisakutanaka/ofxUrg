...
bool Device::open() {
  if (!urg.open(device_or_ip_name.c_str(), baudrate_or_port_number,
                (connect_type == SERIAL) ? qrk::Urg_driver::Serial
...
bool Device::start() {
  ...
  active = urg.start_measurement(measurement_type,
  ...
  while (isThreadRunning()) {
    if (active) {
      ...
      if (mode == DISTANCE) {
        ...
        success = urg.get_distance_intensity(local_data, local_intensity,
                                             &local_timestamp);
      }
      ...
      if (success) {
        ...
        ofLogNotice() << "stop" << device_or_ip_name;
        urg.stop_measurement();
        ofSleepMillis(1);
        ofLogNotice() << "restart" << device_or_ip_name;
        qrk::Lidar::measurement_type_t measurement_type;
        switch (mode) {
        case DISTANCE:
          measurement_type = qrk::Lidar::Distance;
    ...
    if (r > valid_range_min && r < valid_range_max) {
      ...
      if (world.x > valid_aabb_min.x && world.x < valid_aabb_max.x &&
        ...
        if (current_cluster.points.size() != 0 &&
            (current_cluster.end_point.second - world).length() <
                clustering_dist_th) {
          ...
          current_cluster.centroid =
              f * current_cluster.centroid + (1.0 - f) * world;
